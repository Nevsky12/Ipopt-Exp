...
{
public:
   bool get_nlp_info(Index& n, Index& m, Index& nnz_jac_g, Index& nnz_h_lag, IndexStyleEnum& style) override
   {
      n = 3;
      m = 0;
      nnz_jac_g = 0;
   ...
   bool get_bounds_info(Index, Number* x_l, Number* x_u, Index, Number*, Number*) override
   {
      x_l[0] = 0.;
      x_u[0] = 0.;
      x_l[1] = -1.;
   ...
   bool get_starting_point(Index, bool init_x, Number* x, bool, Number*, Number*, Index, bool, Number*) override
   {
      if( init_x )
      {
         x[0] = x[1] = x[2] = 0.;
   ...
   bool eval_h(
      Index, const Number*, bool, Number obj_factor, Index, const Number*, bool, Index,
      Index* i_row, Index* j_col, Number* values) override
   {
      if( values == nullptr )
      {
         for( Index i = 0; i < 3; ++i )
   ...
   void finalize_solution(
      SolverReturn, Index, const Number*, const Number*, const Number*, Index, const Number*, const Number*, Number,
      const IpoptData*, IpoptCalculatedQuantities*) override
   {
   }

   std::uint64_t ValueEvaluations() const
   {
      return value_evaluations_;
   }

private:
   std::uint64_t value_evaluations_ = 0;
};

struct Sample
{
   double ns_per_call;
   std::uint64_t value_evaluations;
};
...
Sample Run(std::size_t iterations, std::size_t warmup)
{
   ...
   if( !adapter->GetSpaces(
          x_space, c_space, d_space, x_l_space, px_l_space, x_u_space, px_u_space, d_l_space, pd_l_space,
          d_u_space, pd_u_space, jac_c_space, jac_d_space, h_space) )
   {
      throw std::runtime_error("TNLPAdapter space construction failed");
   }

   SmartPtr<Vector> x = x_space->MakeNew();
   SmartPtr<Vector> yc = c_space->MakeNew();
   SmartPtr<Vector> yd = d_space->MakeNew();
   SmartPtr<SymMatrix> hessian = h_space->MakeNewSymMatrix();
   x->Set(0.25);
   yc->Set(0.);
   yd->Set(0.);
   ...
   {
      if( !adapter->Eval_h(*x, 1., *yc, *yd, *hessian) )
      {
         throw std::runtime_error("TNLPAdapter Hessian warm-up failed");
      }
   }
   ...
   for( std::size_t i = 0; i < iterations; ++i )
   {
      if( !adapter->Eval_h(*x, 1., *yc, *yd, *hessian) )
      {
         throw std::runtime_error("TNLPAdapter Hessian evaluation failed");
      }
   }
   const auto stop = std::chrono::steady_clock::now();

   const std::uint64_t expected_evaluations = iterations + warmup;
   if( concrete_problem->ValueEvaluations() != expected_evaluations )
   {
      throw std::runtime_error("unexpected Hessian callback count");
   }

   return {
      std::chrono::duration<double, std::nano>(stop - start).count() / static_cast<double>(iterations),
      concrete_problem->ValueEvaluations()
   };
}
...
int main(int argc, char** argv)
{
   try
   {
      ...
      for( std::size_t repetition = 0; repetition < repetitions; ++repetition )
      {
         ...
         callback_evaluations += sample.value_evaluations;
      }
      std::sort(samples.begin(), samples.end());

      std::cout << std::setprecision(17)
                << "{\"schema_version\":1"
                << ",\"name\":\"tnlp_adapter_eval_h_fixed_variable\""
   ...
   {
      std::cerr << "tnlp_adapter_eval_h_benchmark: " << error.what() << '\n';
      return EXIT_FAILURE;
   }
   return EXIT_SUCCESS;
}
